...
namespace Win.App.Server
{
    public class QuizHub : Hub
    {
        ...
        public void QuestionAnsweredQL2(QuizL2 quiz, string userName)
        {
            ...
            if (answer.Equals(correctAnswer, StringComparison.OrdinalIgnoreCase))
            {
                Program.frmQuizL2Server.UpdateAndReloadScore(userName, 1);//we might even aler the UI to refresh the score
            }
        ...
        public void QuestionAnsweredQL3(QuizL3 quiz, string userName)
        {
            ...
            if (answer.Equals(correctAnswer, StringComparison.OrdinalIgnoreCase))
            {
                Program.frmQuizL3Server.UpdateAndReloadScore(userName, 1);//we might even aler the UI to refresh the score
            }
        ...
        public void QuestionAnsweredQClincher(Clincher quiz, string userName)
        {
            ...
            if (answer.Equals(correctAnswer, StringComparison.OrdinalIgnoreCase))
            {
                Program.frmQClincherServer.UpdateAndReloadScore(userName, 1);//we might even aler the UI to refresh the score
            }
